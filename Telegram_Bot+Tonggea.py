...
path_of_ycs = Path + "/ycs.csv"
Choongsuck = pd.read_csv(path_of_ycs)
bigC=0
for i in range(0,len(Choongsuck)):
    if Choongsuck['label'][i]==1:
        bigC= bigC +1
print('bigC : ',bigC)

path_of_wunhyung = Path + "\욕 원형.csv"

wunhyung = pd.read_csv(path_of_wunhyung)

OG=list()

for i in range(0,len(wunhyung)):
    OG.append(wunhyung['Word'][i])

print(OG)

def Opitimze_message(text):
    ##bot.send_message(chat_id=update.message.chat_id, text=update.message.text)
    #target = Finder(my_text)

    #bot.send_message(chat_id=update.message.chat_id, text=target)
    print(text,"is it ok?")
    test = return_bad_words_index(text, mode=1)
    print(test,"test%%%%")
    try:
        print(test[0][1])
        target_word = str(test[0][1])

        if target_word in OG:
